...
export default function ProductCard({ product, meta, onTap, disableCart = false }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 shrink-0">
        ...
        <button 
          type="button"
          onClick={disableCart ? (e) => e.stopPropagation() : handleAddToCart}
          disabled={disableCart}
          aria-label={disableCart ? "Adăugare în coș dezactivată" : "Adaugă în coș"}
          className={[
            "relative p-1.5 rounded-md transition-colors",
            disableCart
              ? "text-zinc-600 opacity-50 cursor-default"
              : "text-zinc-400 hover:text-white hover:bg-zinc-800 active:bg-zinc-700"
          ].join(' ')}
        >
          <ShoppingCart size={18} />
          {!disableCart && quantityInCart > 0 && (
