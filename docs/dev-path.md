...
- **Feature (Smart JIT Data Sync)**: Implementat triggere de sincronizare invizibilă a datelor în `SpacePage.jsx` (`handleIntentSync`). Acum `deltaFetchSpaceProducts` și `deltaFetch` (pentru flux) se declanșează automat la revenirea în focus a ferestrei (`visibilitychange`, `focus`) și la intenția de acțiune (când utilizatorul deschide filtrele din BottomBar sau Meniul de Context). Aceasta înlocuiește necesitatea unui buton de refresh manual sau a unui polling continuu, garantând date 100% actualizate exact la momentul luării deciziilor (ex. filtrare).
...
- **UX & Architecture (Space Filter, BottomBar, Backdrop Click)**:
  ...
  - **Space Filter Context Menu**: Meniul contextual din spațiu folosește acum un prop `trailingAction` (adăugat în `ContextMenu.jsx`) pentru rândul de „Filtrare”. Acest rând oferă 2 ținte de acțiune: click-ul pe text deschide foaia de filtru, iar click-ul explicit pe bifa din dreapta funcționează ca un Reset Global (curăță instantaneu setările de filtre).
  - **Decuplare Reset Atribute vs. Perioadă**: Butonul de calendar din `FluxFilterSheet` acum închide fereastra curentă și deschide un `IntervalSheet` dedicat. Butonul „Resetează” din foaia principală curăță DOAR atributele/categoriile, păstrând intactă perioada de timp. Butonul „Resetează” proaspăt adăugat în `IntervalSheet` curăță doar perioada. Navigarea între spații păstrează nativ filtrele setate, grație arhitecturii Zustand bazate pe `useFluxStore`.
- **Bug Fix & UX (FluxFeed & SpacePage)**: 
  - **FluxBlock Footer Alignment**: S-a eliminat inversarea flexibilă (`flex-row-reverse`) de pe footer-ul cardurilor de ieșire. Acum, atât la intrări cât și la ieșiri, numărul de articole rămâne constant aliniat în stânga (sub lista de nume produse), iar totalul de bucăți rămâne în dreapta (sub lista de cantități).
  - **Infinite Loop Fix**: S-a rezolvat eroarea critică `Maximum update depth exceeded` cauzată de competiția pe starea globală de autocomplete fantomă. Când o tranzacție era deschisă, atât `SpacePage` (via `usePicker`) cât și `TransactionSheetContent` (via `useBottomSearch`) încercau simultan să seteze textul sugerat. S-a introdus parametrul `enabled` în `usePicker` și s-a dezactivat căutarea fundalului (`SpacePage`) pe modul Flux. De asemenea, funcția inline `labelFn` din `TransactionSheetContent` a fost extrasă pentru a preveni invalidarea referinței la fiecare randare.
  - **FAB Hiding**: S-a ascuns automat butonul plutitor (FAB) de filtrare a fluxului atunci când este deschis dialogul cu detaliile unei tranzacții, pentru a preveni crearea unei stive confuze de modale (dialog peste dialog) și pentru a menține focusul utilizatorului pe conținutul tranzacției selectate. S-a realizat printr-o nouă stare `isTransactionSheetOpen` în `SpacePage`, alimentată prin noul prop `onSheetOpenChange` din `FluxFeed`.
- **UI/UX (FluxFeed BottomSheet)**: 
  - Extins componenta `FluxFeed.jsx` pentru a permite deschiderea detaliilor oricărei tranzacții (`FluxBlock` sau `SummaryBlock`) într-un `BottomSheet` (în loc de un footer expandabil inline).
  - Footer-ul original al tranzacției a devenit pur vizual (read-only), afișând în stânga numărul total de elemente și în dreapta suma produselor.
  - Odată cu afișarea în BottomSheet, s-a activat hook-ul `useBottomSearch` pe lista de elemente, oferind filtrare instantanee nativă prin BottomBar (ex. poți căuta un produs anume direct în lista de produse a unei recepții).
  - Câștig imens de spațiu vizual în listă (fat-finger friendly) și păstrarea curată a feed-ului, limitat acum permanent la un preview maxim de 3 produse.
- **A11y Fix (BottomBar Search Input)**: S-au rezolvat erorile de accesibilitate și validare HTML pe inputul de căutare/adăugare tag-uri din `BottomBar.jsx`. S-a atașat un element `<label className="sr-only">` pentru screen readere, iar atributele `id` și `name` au fost schimbate din generic-ul `"search"` în `"bottom-bar-search"` pentru a preveni coliziuni și interferențe cu extensii de autofill/password managere. Referințele globale de focus (`document.getElementById`) au fost corectate corespunzător.
- **Bug Fix (PickerSheet Search)**: S-a adăugat proprietatea explicită `searchContext: 'picker_sheet'` la apelul hook-ului `usePicker` din interiorul `PickerSheet.jsx` și s-a înlocuit preluarea globală a textului din `useAppStore` cu noul `useActiveSearchQuery('picker_sheet')`. Această modificare reconectează picker-ul la bara de căutare din BottomBar, rezolvând problema filtrării introduse neintenționat odată cu refactorizarea sistemului de placeholders/context din commit-ul `busuioc`.

### [Commit fae752f] — build: diamant | commit: cascada - feat/fix: setup i18n react-i18next & BottomBar search placeholders 100% localization
- **Code Cleanup & i18n Fallback (PickerSheet)**: S-a curățat codul mort din `PickerSheet.jsx` (prop-ul `restorePlaceholder` rămas de la vechea arhitectură imperativă). De asemenea, s-a eliminat valoarea implicită hardcodată (`searchPlaceholder = 'Caută...'`). Acum, dacă părintele nu transmite un placeholder, `PickerSheet` extrage automat fallback-ul dinamic din dicționar prin `t('search.placeholder_fallback')`. Logica din `useEffect` a fost separată curat (inițializare vs. context de căutare) pentru a suporta reactivitatea la schimbarea limbii.
- **i18n Placeholders Finalization (Varianta A - 100% Acoperire BottomBar)**: S-au eliminat ultimele placeholders hardcodate din aplicație. În `ProductFormSheet.jsx`, selectorul de Tags folosește acum `t('search.tag_picker')` (*"Search or add tag..."* / *"Caută sau adaugă tag..."*), selectorul de atribute folosește `t('search.attr_picker', { name })` (*"Search {{name}}..."* / *"Caută {{name}}..."*), iar prop-ul rezidual `restorePlaceholder` a fost curățat. În `BottomBar.jsx`, căutarea după Name ID a fost conectată la `t('search.name_id')`. Astfel, bara de căutare este 100% localizată în orice context din aplicație.
- **i18n Sheet Placeholders (Parametric & Dynamic)**: S-au adăugat în dicționarele `ro.json` și `en.json` cheile de căutare parametrică (`filter_dimension` cu variabila `{{name}}`, `filter_options` și `destination_picker`). În `BaseFilterSheet.jsx`, placeholder-ul se generează acum conform principiului stabilit: ghidajul de acțiune se traduce (`Search in {{name}}...` / `Caută în {{name}}...`), în timp ce denumirea atributului definit de utilizator rămâne intactă. De asemenea, `DestinationPicker.jsx` a fost migrat la `useTranslation`.
- **Bug Fix (i18n Placeholders)**: Paginile principale (`CatalogPage`, `CategoryPage`, `StockHubPage`, `SpacePage`) împingeau textul hardcodat în română către `BottomBar`, suprascriind comportamentul de fallback. Au fost introduse cheile specifice în `ro.json` / `en.json` și s-a folosit `useTranslation` în interiorul hook-urilor `useEffect` din aceste pagini. Acum, când utilizatorul schimbă limba, hook-urile se re-execută nativ și trimit textul tradus către bara de jos, permițând actualizarea instananee a placeholder-ului (ex: `Search category or folder...`).
- **Internationalization (i18n Setup)**: S-a instalat și configurat infrastructura de bază pentru traduceri (`i18next`, `react-i18next`). S-au creat fișierele de configurare și dicționarele inițiale (`ro.json`, `en.json`). S-a tradus meniul lateral (`SideMenu`) folosind chei de dicționar (`labelKey`) în loc de texte hardcodate și s-a adăugat un buton de comutare a limbii. De asemenea, s-a interceptat textul implicit `"Caută..."` din `BottomBar` pentru a preluat din noul dicționar. Aceasta este fundația pentru traducerea integrală viitoare.
- **feat: Flux Filtering Architecture — Faza 1 (Local, Working Window + Session Cache)**:
  - `src/store/useFluxStore.js` **[NOU]**: Store Zustand dedicat pentru tab-ul Flux. Implementează Working Window (cele mai recente 2.000 tx offline-first), Session Cache (fetch brut pentru perioade cu COUNT ≤ 2.000), routing pe 3 niveluri (local WW / local SC / `volume_exceeded`), delta-fetch post-commitCart bazat pe cursor `created_at` (nu UUID), infinite scroll paginate (50 tx/pagină), agregare locală per granularitate (zilnic/săptămânal/lunar). Store-ul este complet separat de `useStockStore` și `useCatalogStore`.
  - `src/components/stockhub/FluxFeed.jsx` **[MODIFICAT]**: Extins cu `SummaryBlock` (bloc sumar pentru granularitate agregată), prop `mode` (`'transaction'` | `'aggregated'`), și `IntersectionObserver` pentru infinite scroll. Componentele existente `FluxBlock` și `AlertBlock` rămân neatinse funcțional.
  - `src/components/stockhub/FluxFilterSheet.jsx` **[NOU]**: Bottom-sheet dedicat filtrării Flux, complet separat de `FilterSheet.jsx`. Header interactiv cu `PeriodPopover` (Perioadă + Granularitate), filtre: Tip Tranzacție (chips verde/roșu), Spațiu Partener (`PickerSheet` v2), Categorie (chips), Tags, Atribute dinamice per categorie. Starea se scrie în `useFluxStore.fluxFilter` (nu în `useCatalogStore`).
  - `src/pages/SpacePage.jsx` **[MODIFICAT]**: Integrat `useFluxStore` (înlocuiește `fetchSpaceTransactions` + `fluxBlocks` local state), `clearFlux()` la unmount, `deltaFetch()` ready (apelabil post-commitCart), banner stări speciale (`offline_blocked`, `volume_exceeded`, `error`), `FluxFilterSheet` conectat, Context Menu actualizat cu opțiunea „Filtrare Flux" (cu badge „Activ") când view='flux'.
  - `docs/specs/SPEC_Space_Filtering_Architecture.md` **[MODIFICAT]**: Adăugat notice de deprecare la §3 cu barare vizuală (`~~`); §1–§2 rămân valide.
- **Fix/Refactor (FluxFilterSheet Picker)**: Am decuplat componenta `PickerSheet` (folosită pentru selecția Spațiului Partener) de logica internă de UUID. Picker-ul primește și randează acum strict array-uri de denumiri de spații (`s.name`). La momentul confirmării selecției (`onConfirm`), `FluxFilterSheet` se ocupă de *reverse-mapping* din string în UUID, folosind o rutină de *Sanity Check* (`filter`) ce emite un avertisment clar în consolă în cazul improbabil în care s-ar detecta spații cu denumiri duplicate.
- **Bug Fix (React Hooks Rule in FluxFeed)**: Corectat eroarea critică "*Rendered fewer hooks than expected*" generată la comutarea granularității pe modul agregat (ex: „Zi”). Am mutat declarația hook-ului `useMemo` (care procesează lista de tranzacții) înaintea blocurilor de cod cu `return` rapid folosite pentru modul `aggregated`. Pentru a preveni calcule inutile, `useMemo`-ul face acum un short-circuit intern (`return []`) dacă modul nu este `transaction`, păstrând compatibilitatea strictă a framework-ului React privind ordinea și numărul de apeluri ale hook-urilor.
- **Bug Fix (Aggregation Bypass in Infinite Scroll)**: Corectat logica funcției `_recomputeResult` din `useFluxStore` care sărea complet peste funcția de agregare (`aggregateByGranularity`) atunci când filtrul de perioadă era gol (modul Infinite Scroll / Working Window). Aceasta cauza trimiterea de tranzacții brute către un UI care aștepta block-uri agregate, rezultând în carduri goale la setarea granularității pe o listă completă. Funcția unifică acum fluxul de filtrare și agregare indiferent de sursa datelor.
- **UI/UX (FluxFilterSheet 2-Column Layout Redesign)**: Rescris complet interfața de filtrare `FluxFilterSheet` pentru a adopta designul consacrat pe 2 coloane, orchestrat de componenta principală `BaseFilterSheet`. 
  - S-au adăugat props noi `headerExtension` și `showCounts={false}` în `BaseFilterSheet` pentru a susține excepțiile din vizualizarea de flux.
  - S-a decuplat selecția de „Spațiu Partener” din modalul separat (`PickerSheet`) și s-a introdus ca dimensiune nativă în interfața pe 2 coloane (se integrează nativ cu căutarea BottomBar).
  - Selectorul de Perioadă și Granularitate (`PeriodPopover`) a fost integrat direct în header-ul filtrului.
  - **Faceted Extraction Inteligent (Nou)**: Elementele din coloana stângă (Spațiu Partener, Categorie, Tags, Atribute) și valorile din coloana dreaptă sunt acum filtrate *dinamic*. Se afișează doar spațiile, categoriile, tag-urile și atributele care chiar există în sursa brută de date (în fereastra curentă de 2000 tranzacții), ascunzând complet orice filtru care ar fi dus la o listă goală (0 rezultate).
  - **Flux Filter Toggle Mode**: Opțiunea din meniul de context funcționează acum ca un "checkbox". Când este activată:
    1. Afișează un *Floating Action Button* (FAB) deasupra listei pentru a ascunde/arata panoul rapid.
    2. Modifică textul gri "11 tranzacții recente" într-un text albastru informativ "Filtrate x/y tranzacții", unde `x` este numărul rezultatelor brute neagregate ce trec de filtru, iar `y` este totalul ferestrei curente (via noul element din stare `filteredRawCount`).
    3. Dezactivarea opțiunii din meniu ascunde butonul și curăță instantaneu orice filtru aplicat, revenind la fluxul normal.
    4. Adaugă opțiunea "Interval" în meniul de context care deschide doar modalul de selectare a perioadei.
  - **Filtrare Produse per Tranzacție**: S-a adăugat posibilitatea de a ascunde sau afișa produsele nefiltrate dintr-o tranzacție mixtă. O opțiune "Toată tranzacția" (în PeriodPopover) controlează acest comportament. Produsele individuale care corespund criteriilor primesc vizual o bifă (Check mark) în interfața de feed și în modurile de agregate.
  - **HOTFIX: Faceted Extraction Values**: S-a reparat un bug din `FluxFilterSheet` unde valorile din dreapta pentru Tag-uri și Atribute apăreau goale deși dimensiunea exista, decuplându-se complet de indexul global al catalogului (care putea fi neîncărcat) și preluând opțiunile vizibile direct din analiza brută locală.
  - **HOTFIX: UI BottomBar FilterSheet**: S-a ajustat comportamentul noului buton plutitor (FAB) pentru a fi ascuns automat când meniul de context este deschis, prevenind deschiderea simultană a mai multor modaluri care cauzau acoperirea incorectă a barei de navigare de jos. De asemenea, `IntervalSheet` respectă acum `aboveBottomBar=true`.
  - **Single Active Space Cache & Delta Fetch**: S-a eliminat încărcarea neagră vizibilă la întoarcerea din pagina produsului (`ProductPage`) înapoi în `SpacePage`. Acum, spațiul curent e salvat în RAM (`useStockStore.activeSpaceCache`). La navigarea "Back", interfața se randează instantaneu din memorie, declanșând doar o verificare ușoară în fundal (Delta Fetch) a stocurilor modificate peste baza de timp a `updated_at`, respectând 100% standardul Local-First.
  - **Smart Scroll Restoration (Zero-Overhead)**: S-a rezolvat problema revenirii la începutul listelor lungi la apăsarea butonului "Back". Utilizând un mecanism de `scrollCache` în `useAppStore`, aplicația capturează pixelul exact doar în fracțiunea de secundă în care dai tap pe un produs (fără listeners costisitoare pe onScroll). La întoarcere (atât în `SpacePage` cât și în `CategoryPage`), poziția este restaurată perfect cu `useLayoutEffect`, înainte ca ecranul să se redea, oferind o senzație de continuitate fluidă.
  - **HOTFIX (Infinite Loop SWR)**: S-a identificat și remediat un bug sever de re-randare infinită în `SpacePage.jsx`. Din cauza modului în care React evalua funcția `t()` (i18next) și actualizările din `useAppStore` ca fiind referințe noi, mecanismul de delta-fetch (SWR) se curăța și se relua la fiecare câteva milisecunde, generând sute de cereri blocate în modul Offline. S-a izolat `useEffect`-ul să asculte strict de schimbarea ID-ului spațiului curent.
  - **SWR Cache Throttling (TTL)**: S-au adăugat limite de timp (TTL - Time-To-Live) pentru a preveni spam-ul de rețea (SWR pinging) când utilizatorul navighează rapid Back/Forward între pagini: 60s pentru detaliile complete ale produselor (`ProductPage`) și 15s pentru `deltaFetchSpaceProducts` și `fetchAlerts` (`SpacePage`). În plus, cache-ul Flux nu mai este șters la navigarea "Back" din `SpacePage`, permițând redarea instantanee a istoricului din RAM la întoarcere.
  - O experiență de utilizare unificată cu filtrele din secțiunea de Catalog/Stoc!
### [Commit 474a6f7] — build: diamant | commit: clepsidra - feat: SideMenu latime dinamica dupa continut
